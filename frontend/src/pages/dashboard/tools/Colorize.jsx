...
export default function Colorize() {
  ...
  const run = async () => {
    if (!photo) { toast.error("Envia uma foto preto-e-branco."); return; }
    setBusy(true); setResult(null);
    try {
      const fd = new FormData();
      fd.append("photo", photo);
      const { data } = await api.post("/tools/colorize", fd, { timeout: 180000 });
      setResult(data.creation);
      toast.success(`Colorido · ${data.creation.credits_spent} créditos`);
      await refresh();
  ...
  return (
    <ToolFrame
      testId="colorize"
      title="Colorize B&W"
      subtitle="Transforma fotografias a preto-e-branco em imagens a cores realistas."
