...
const { ensureMarketplaceIndexes, listChats, addChatMessage } = require("../lib/marketplaceDb.cjs");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const messages = await listChats(identity, listingId);
      return json(res, 200, { ok: true, messages });
    }

    if (req.method === "POST") {
      const body = await readBody(req);
      const message = await addChatMessage(identity, body);
