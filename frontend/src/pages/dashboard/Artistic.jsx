...
export default function Artistic() {
  ...
  return (
    <div className="max-w-[1200px] mx-auto" data-testid="artistic-page">
      <p className="eyebrow mb-3">Artistic</p>
      <h1 className="heading-xl mb-10">A photo, <span className="italic text-rp-lavender">reimagined</span>.</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-10">
        <div>
          ...
          <label className="block text-[11px] font-mono uppercase tracking-[0.2em] text-rp-mute2 mb-3">Choose a style (33)</label>
          ...
          <label className="block text-[11px] font-mono uppercase tracking-[0.2em] text-rp-mute2 mb-3">Optional direction (free text)</label>
          <input value={extra} onChange={(e) => setExtra(e.target.value)} placeholder="e.g. warm tones, autumn light…" className="field-input mb-10" data-testid="artistic-extra" />
          ...
          <button onClick={generate} disabled={busy || !photo || (user?.credits ?? 0) < cost} className="btn-primary w-full disabled:opacity-50" data-testid="artistic-generate">
            {busy ? (<><Loader2 className="w-4 h-4 animate-spin" /> Generating…</>) : (<><Palette className="w-4 h-4" /> Transform · {cost} credits</>)}
          </button>
        </div>

        <aside className="lg:sticky lg:top-[88px] self-start">
          <p className="eyebrow mb-4">Last result</p>
          <ResultPanel creation={result} loading={busy} onChange={setResult} emptyLabel="Upload a photo, pick a style." />
        </aside>
      </div>
    </div>
  );
