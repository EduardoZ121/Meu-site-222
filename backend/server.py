...
from services.aurora import aurora_generate  # noqa: E402
...
# ============== Aurora static file server ==============
# Serve images generated by Aurora (Nano Banana) from /api/static/aurora/{fname}.
# Files live in /app/backend/uploads/aurora/ — written by services/aurora.py.
from fastapi.responses import FileResponse  # noqa: E402

@api.get("/static/aurora/{fname}")
async def _aurora_static(fname: str):
    # Strict allow-list of jpg files in the aurora folder; no path traversal.
    if not fname.endswith((".jpg", ".png", ".webp")) or "/" in fname or ".." in fname:
        raise HTTPException(status_code=400, detail="Bad filename")
    fpath = Path("/app/backend/uploads/aurora") / fname
    if not fpath.exists():
...
async def generate_edit(
    ...
    engine: str = Form("grok"),  # "grok" (Replicate, 12 cr) | "aurora" (Nano Banana, 15 cr)
    photo: UploadFile = File(...),
    current=Depends(get_current_user),
):
    """Edit a photo with a free-text prompt. Returns `prediction_id` for polling
    when engine=grok; returns the finished `creation` directly when engine=aurora.
    """
    if engine not in ("grok", "aurora"):
        raise HTTPException(status_code=400, detail="Unknown engine")
    cost = 15 if engine == "aurora" else COSTS.get("pro_base", 12)
    ...
    if engine == "aurora":
        # Synchronous Aurora generation — typically 5-12s, well under the
        # K8s ingress timeout, so no polling pattern needed.
        new_balance = await _spend_credits(current["sub"], cost, "Edit photo (Aurora)")
        try:
            urls = await aurora_generate(
                prompt=final_prompt, image_paths=[photo_path],
                num_outputs=1, aspect_ratio=aspect_ratio,
            )
        except Exception as e:
            new_balance = await _add_credits(current["sub"], cost, "refund", f"Refund: aurora failed ({e})")
            cleanup(photo_path)
            raise HTTPException(status_code=502, detail=f"Aurora failed: {str(e)[:200]}")
        finally:
            cleanup(photo_path)
        creation = Creation(
            user_id=current["sub"], type="image", model_used="aurora",
    ...
    # Default: Grok via Replicate (async polling)
    try:
        pending = await _create_pending(
            ...
            spend_description="Edit photo (Grok)",
        )
    finally:
        cleanup(photo_path)
    return {
...
async def generate_easy(
    ...
    engine: str = Form("grok"),  # "grok" (default, 11 cr) | "aurora" (15 cr)
    photo: UploadFile = File(...),
    current=Depends(get_current_user),
):
    ...
    cost = 15 if engine == "aurora" else (COSTS.get("fast_base", 10) + COSTS.get("fast_style_extra", 1))
    ...
    if engine == "aurora":
        new_balance = await _spend_credits(current["sub"], cost, f"Easy Aurora ({style_id})")
        try:
            urls = await aurora_generate(
                prompt=raw_prompt, image_paths=[photo_path],
                num_outputs=1, aspect_ratio=aspect_ratio,
            )
        except Exception as e:
            new_balance = await _add_credits(current["sub"], cost, "refund", f"Refund: aurora failed ({e})")
    ...
    try:
        pending = await _create_pending(
            ...
            aspect_ratio=aspect_ratio, num_outputs=1, image_path=photo_path,
            style_key=style_id, spend_description=f"Easy Grok ({style_id})",
        )
    finally:
        cleanup(photo_path)
    return {
...
POSTER_MODEL_COSTS = {
    "grok":      15,
    "aurora":    18,   # was "flux2" — Aurora = Nano Banana (cheaper to us, premium UI)
    "gpt_image": 25,
}


def _poster_model_to_replicate_key(model_key: str) -> str:
    """Map UI model_key to replicate_service MODELS key. Aurora bypasses Replicate."""
...
async def generate_poster_route(
    ...
    try:
        ...
        if model_key == "gpt_image":
            ...
            # Aurora (Nano Banana) — no Replicate, no waiting
            ...
            if not urls:
                raise RuntimeError("Empty image from Aurora")
            model_label = "aurora"
        else:
...
async def public_poster_models():
    ...
    return {
        "models": [
            ...
            {"key": "aurora",    "label": "Aurora",        "cost": POSTER_MODEL_COSTS["aurora"],    "tier": "pro",     "supports_photo": True,  "tag": "Foto-realista · IA neural"},
