...
import { Link2, X, ArrowRight, Pencil, Zap } from "lucide-react";

export default function ConnectionPromptModal({ source, target, initialPrompt, isEditing, onConfirm, onCancel }) {
      ...
      <div className="manga-flow-modal manga-flow-modal--connection" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="manga-flow-conn-input-wrap">
          ...
          <textarea
            ...
            placeholder="e.g. 'They are hugging under the rain', 'Looking at each other with anger'..."
            className="manga-flow-conn-textarea"
            rows={3}
            autoFocus
...
function getSuggestions(sourceType, targetType) {
  ...
  if (s === "person" && t === "person") {
    return [
      "They are talking", "They are fighting", "They are hugging",
      "Walking together", "Chasing the other", "Arguing intensely",
      "Looking at each other", "Standing back to back",
      "Shaking hands", "Protecting the other",
    ];
  }
  if (s === "person" && t === "scenario") {
    return [
      "Standing here", "Walking through", "Running away from",
      "Arriving at", "Hiding in", "Looking at the view",
      "Sitting inside", "Falling from", "Flying over",
    ];
  }
  if (s === "person" && t === "object") {
    return [
      "Holding it", "Using it", "Looking at it",
      "Throwing it", "Reaching for it", "Dropping it",
      "Breaking it", "Receiving it", "Hiding it",
    ];
  }
  if (s === "person" && t === "speech") {
    return [
      "Saying this", "Thinking this", "Screaming this", "Whispering this",
    ];
  }
  if (s === "person" && t === "effect") {
    return [
      "Surrounded by effect", "Generating this effect", "Hit by this effect",
    ];
  }
  if (s === "person" && t === "camera") {
    return [
      "Is the focus of this shot", "Framed by this angle",
    ];
  }
  if (s === "scenario" && t === "person") {
    return [
      "Person is inside", "Person is entering", "Person is leaving",
      "Person is trapped here", "Person is observing from here",
    ];
  }
  if (s === "effect" && t === "person") {
    return [
      "Hitting the character", "Surrounding the character", "Following the character",
    ];
  }
  if (s === "camera" && t === "person") {
    return [
      "Focusing on this character", "Following this character",
    ];
  }
  if (s === "panel" || t === "panel") {
    return [
      "Belongs to this panel", "Placed inside this frame", "Fills this panel",
    ];
  }
