#!/usr/bin/env node
/**
 * Gera capas JPG para a grelha Stories (Personalizar / Estúdio).
 * Ref feminina — posts editoriais Instagram (awareness, testimonial, wellness).
 *
 * Uso: node scripts/generate-stories-covers.mjs
...
const STORY_JOBS = [
  { id: "st_awareness", aspect_ratio: "3:4" },
  { id: "st_testimonial", aspect_ratio: "3:4" },
  { id: "st_wellness", aspect_ratio: "3:4" },
];
...
function loadStoryStyles() {
  ...
  return Object.fromEntries(styles.filter((s) => s.cat === "stories").map((s) => [s.id, s]));
}
...
async function main() {
  ...
  console.log("\nConcluído. Entradas st_* em padraoStyleCovers.js.");
}
