      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-[64px] border-b border-rp-border bg-rp-bg/80 backdrop-blur-xl sticky top-0 z-30 flex items-center justify-between px-6 lg:px-10">
          ...
          <div className="ml-auto flex items-center gap-5">
            ...
            <Link
              to="/app/profile"
              className="rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rp-purple/50 focus-visible:ring-offset-2 focus-visible:ring-offset-rp-bg"
              data-testid="header-avatar"
              title={t("sidebar_profile")}
            >
              <Avatar className="h-9 w-9 border border-rp-border bg-rp-surface shadow-sm ring-1 ring-black/20 transition hover:border-rp-purple/40 hover:ring-rp-purple/20">
                {user.avatar_url ? <AvatarImage src={user.avatar_url} alt="" className="object-cover" /> : null}
                <AvatarFallback className="bg-rp-surfaceRaised text-[11px] font-mono font-medium text-rp-text uppercase tracking-tight">
                  {headerInitials(user)}
                </AvatarFallback>
              </Avatar>
            </Link>
