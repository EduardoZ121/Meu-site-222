...
async function submitInstantPosterGeneration(req, fields, {
  ...
  if (storageEnabled() && user?.id && !isLocal) {
    ...
    if (!isAdminEmail(dbUser?.email)) {
      newBalance = await spendCredits(user.id, cost, "Pôster");
    }
    ...
    try {
      ...
      if (!isAdminEmail(dbUser?.email)) {
        await addCredits(user.id, cost, "refund", `Refund: poster sync failed (${String(e.message || e).slice(0, 80)})`);
      }
      const err = new Error(formatGenerationError(e.message || "submit failed", lang));
      err.status = 502;
      throw err;
  ...
  const meta = withMeta({ prompt }, cost, "poster");
  return {
...
async function imageInput(fields, files, modelKey, prompt, opts = {}) {
  ...
  if (
    ...
    primary = await fitImageRefToAspect(primary, aspectRatio);
  }
  const input = {
