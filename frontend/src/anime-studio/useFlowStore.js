...
import { DEFAULT_SCENES } from "./animeStudioCatalog";

const STORAGE_KEY = "rp_anime_studio_project";
...
function flushSceneGraph(scenes, activeSceneId, nodes, edges) {
  return scenes.map((sc) =>
    sc.id === activeSceneId ? { ...sc, nodes, edges } : sc,
  );
}
...
export const useFlowStore = create((set, get) => ({
  ...
  scenes: initial.scenes?.length ? initial.scenes : buildDefaultScenes(),
  activeSceneId: initial.activeSceneId || initial.scenes?.[0]?.id || DEFAULT_SCENES[0].id,
  historyPast: [],
  historyFuture: [],
  selectedNodeId: null,
  selectedEdgeId: null,
  ...
  pushHistory: () => {
    ...
    const past = [...historyPast, snap].slice(-30);
  ...
  undo: () => {
    ...
    try {
      ...
      set({
        ...
        historyFuture: [current, ...historyFuture].slice(0, 30),
        nodes: parsed.nodes,
        edges: parsed.edges,
      });
      persist(get());
  ...
  redo: () => {
    ...
    try {
      ...
      set({
        historyPast: [...historyPast, current].slice(-30),
  ...
  setActiveScene: (sceneId) => {
    const s = get();
    const scenes = flushSceneGraph(s.scenes, s.activeSceneId, s.nodes, s.edges);
  ...
  addScene: (name) => {
    ...
    const scenes = flushSceneGraph(s.scenes, s.activeSceneId, s.nodes, s.edges);
    set({
      scenes: [...scenes, { id, name: name || "Nova cena", nodes: [], edges: [] }],
      activeSceneId: id,
      nodes: [],
      edges: [],
      selectedNodeId: null,
  ...
  importProjectJson: (json) => {
    try {
      ...
      const scenes = data.scenes?.length ? data.scenes : buildDefaultScenes();
      const activeSceneId = data.activeSceneId || scenes[0].id;
      const active = scenes.find((sc) => sc.id === activeSceneId) || scenes[0];
      set({
        projectId: data.id || uid("proj"),
        projectName: data.name || "Anime Studio",
        scenes,
        activeSceneId: active.id,
        nodes: active.nodes || [],
        edges: active.edges || [],
        globalSettings: { ...get().globalSettings, ...data.globalSettings },
        story: data.story ? { ...DEFAULT_STORY, ...data.story } : { ...DEFAULT_STORY },
        selectedNodeId: null,
        selectedEdgeId: null,
        historyPast: [],
        historyFuture: [],
  ...
  addNode: (type, position, extra = {}) => {
    ...
    const data = {
      ...
      ...base,
      ...extra,
      name: extra.preset || extra.name || base.name,
    };
    if (extra.role) data.role = extra.role;
    if (extra.speechType) data.speechType = extra.speechType;
    if (extra.effectType) data.effectType = extra.effectType;
    const node = {
      id,
      type: "flowNode",
